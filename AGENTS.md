...
- **Le retour au fil doit dépiler l'historique, pas naviguer vers `/`.** `scrollRestoration` est activé dans `src/router.tsx` et ne s'applique qu'aux retours : un `<Link to="/">` empile une nouvelle entrée, qui démarre au sommet de la page, et la position de défilement est perdue (mesuré le 2026-08-13 : 4750 px restaurés par le bouton du navigateur, 0 par le lien). D'où `src/components/BackButton.tsx`, et son repli obligatoire — arrivé par une notification push sur `/article/:id`, il n'y a rien à dépiler et `back()` sortirait de l'app.
