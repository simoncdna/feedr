				...
				{tabs.map((tab) => {
					...
					return (
						<Link
							...
							className={`flex flex-1 flex-col items-center gap-1 py-2 transition-[color,transform] duration-100 active:scale-[0.94] motion-reduce:transition-none motion-reduce:active:scale-100 ${active ? 'text-accent' : 'text-muted hover:text-foreground'
								}`}
						>
							{tab.icon}
